...
object IPSearchDemo {
  def main(args: Array[String]): Unit = {
    ...
    // 获取ip基础数据并切分
    val ipInfo = sc.textFile("C:\\Users\\26950\\Desktop\\test\\input\\spark\\SparkCore\\ipsearch\\ip.txt")
      .map(line => {
    ...
    val broadcastIPInfo: Broadcast[Array[(String, String, String)]] =
      sc.broadcast(ipInfo.collect)  // 广播变量不能是RDD类型，必须是数组类型

    // 获取用户点击流日志并切分, 根据二分查找到用户ip对应的省份，返回(省份， 1)， 便于后期聚合
    val logs = sc.textFile("C:\\Users\\26950\\Desktop\\test\\input\\spark\\SparkCore\\ipsearch\\http.log")
      .map(line => {
        ...
        val index = binarySearch(ipInfoArr, userIP_Long)  // 由于ip都在范围内，所以未作-1下标判断 !!!
    ...
    // 统计省份对应的访问量
    val aggred: RDD[(String, Int)] = logs.reduceByKey(_ + _)
